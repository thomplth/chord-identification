...
from utility import result_roman_to_jazz
...
class Metric:
    ...
    def parse_result(self, filename, seventh=True):
        ...
        for row in data:
            try:
                chord = result_roman_to_jazz(row[1], row[2])
                offset = float(row[0])
            except ValueError:
                x, y = map(int, row[0].split('/'))  # int or float? int now
                offset = x / y
            self.result.append((chord, offset))
    ...
    def parse_ground_truth(self, filename):
        ...
        for chord, offset in data:
            scale = self.convert_chord_name(chord)
            self.ground_truth.append((scale, offset))
    ...
    def convert_chord_name(self, chord):
        ...
        if len(scale) > 1:
            if scale[1] == 'b':
                ...
                raise Exception(f'Error handling chord name {chord}.')
        ...
        try:
            return result_roman_to_jazz(numeral, scale)
        except Exception as e:
            print(e)
            return scale
...
if __name__ == "__main__":
    scores = [f[:-4]
              for f in os.listdir('../data/') if os.path.isfile('../data/' + f)]
    scores.remove(
    ...
    for k, v in d.items():
        path = f'../result/evaluation_jazz/{k}.csv'
